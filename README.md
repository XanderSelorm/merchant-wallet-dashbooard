...
The value of one shared expression is that the two sides cannot drift when the rate or the rounding rule
changes, not that the naive version is currently wrong.
...
Built with AI assistance (Claude Code) for scaffolding, review, verification, test generation and optimization.
