...
A merchant wallet and settlement dashboard for a fintech platform: register merchants, simulate customer payments with a 1.5% processing fee, watch wallet balances accrue, and sweep those balances into settlements.
...
147 tests across the services, stores and composables:

| Area | Covers |
| --- | --- |
| `lib/fees` | Fee parity against a table generated from the PHP, half-pesewa rounding, amount parsing, reference format |
| `lib/money` | Currency and date formatting, the split-figure treatment recombining to the same string |
| `lib/http` | Mapping a Laravel 422 to per-field messages; leaving other failures to form-level handling |
| `stores/*` | Query params sent for every filter, in-place status patching across list/options/current, settlement run counts, loading flags released on failure |
| `composables/*` | Debounce collapsing a burst of typing, toast lifecycle and auto-dismiss, CSV download naming and object-URL cleanup |

Timezone is pinned to UTC in `vitest.config.ts` so date assertions hold on any machine.

Components are deliberately out of scope — this suite covers the logic, and the UI was verified by
...
units). Conversion to a decimal happens only at the display edge. This removes the entire class of rounding drift that shows up when currency is held in floats.
...
The frontend test suite asserts this against a parity table generated by running the PHP
implementation, including the exact `.5` boundaries.

To be precise about why: this is a structural guarantee, not a fix for a defect I hit. I swept every
amount the API accepts — 1 to 100,000,000 pesewas — and found **no** input where `Math.round(gross *
0.015)` gives a different answer. The value of one shared expression is that the two sides cannot
drift when the rate or the rounding rule changes, not that the naive version is currently wrong.
...
what _would_ have been credited, not what was.
...
or negative balance are skipped, per the requirement that settlement moves _positive_ balances.
...
1. **Simulated payments always succeed.** The payment form records a `successful` transaction. Pending
...
2. **Settlement is platform-wide.** "Run settlement" sweeps every merchant with a positive balance,
   which matches a daily batch process. It can be triggered manually any number of times.
3. **Settlement is a record, not a transfer.** No bank integration; a settlement records that a payout
   was instructed and resets the wallet.
4. **Currency is GHS**, single-currency, formatted `GH₵`.
5. **Inactive merchants cannot receive payments.** Enforced in `PaymentService` and reflected by
   excluding them from the payment form's merchant picker. Their existing balance and history remain
   intact and settleable.
6. **Fees are rounded half-up per transaction.** Summing per-transaction fees therefore differs by a
...
**Verified:** each of the three core journeys — register merchant, simulate payment, run settlement — is completable using only the keyboard.

---

## Known limitations

Things I would do differently or additionally with more time:

- **No component or end-to-end tests.** Vitest covers the services, stores and composables; the components themselves and the full journeys were verified by driving the running app rather than by committed tests. Component tests with Testing Library, and a Playwright run over the three core journeys, are what I would add next.
- **Settlement is synchronous.** A real daily batch belongs in a queued job with a schedule; here it runs in-request so a reviewer can trigger it and see the result immediately.
- **No rate limiting or audit trail** on the settlement endpoint. In production, moving balances would be logged against the operator who triggered it.
- **The 1.5% fee is a constant**, not per-merchant pricing.
- **CSV export streams the full filtered set** with no row cap. Fine at this scale; it would need a queued export and a download link for very large ranges.
- **Merchant records cannot be edited or deleted** — only registered and activated/deactivated, which is what the brief asked for.
...
Built with AI assistance (Claude Code) for scaffolding, review, and verification.
